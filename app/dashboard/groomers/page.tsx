...
export default function GroomersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: groomerRows, error: groomerError }, { data: serviceRows }, { count }] =
      await Promise.all([
        supabase
          .from("groomer")
  ...
  async function handleAddGroomer(e: React.FormEvent) {
    ...
    if (groomerError || !newGroomer) {
      setError(
        groomerError?.message.includes("seat")
          ? groomerError.message
          : groomerError?.message ?? "Could not add groomer."
      );
      setSaving(false);
      return;
    }
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {seatInfo && (
          <p className="text-xs text-[#14261F]/50 mb-8">
            Staff logins used: {seatInfo.used} / {seatInfo.limit} — groomers themselves aren&apos;t
            limited, only who can log in to this dashboard.
          </p>
        )}
        ...
        <div className="space-y-3 mb-8">
          {groomers.length === 0 && (
            <p className="text-sm text-[#14261F]/50 italic">No groomers added yet.</p>
          )}
          ...
          {groomers.map((groomer) => (
            ...
            >
              ...
              <div className="flex flex-col items-end gap-2">
                <button
                  onClick={() => handleDeleteGroomer(groomer.id)}
                  className="text-xs text-red-500 hover:underline"
                >
                  Remove
                </button>
                <a
                  href={`/dashboard/groomers/${groomer.id}/schedule`}
                  className="text-xs text-[#14261F]/60 hover:underline"
                >
                  Schedule exceptions →
                </a>
                <a
                  href={`/dashboard/groomers/${groomer.id}/availability`}
                  className="text-xs text-[#14261F]/60 hover:underline"
                >
        ...
        {/* Add groomer form */}
        <form
            onSubmit={handleAddGroomer}
            className="bg-white border border-black/10 rounded-2xl p-6 space-y-5"
          >
            ...
            <div>
              <label className="block text-sm font-medium text-[#14261F] mb-2">
                Working days &amp; hours
              </label>
            ...
            <div>
              ...
              {services.length === 0 ? (
                <p className="text-xs text-[#14261F]/50 italic">
                  Add services first on the Services &amp; pricing page.
                </p>
              ) : (
                <div className="flex flex-wrap gap-2">
